...
for i in range(10):
  ...
  bc_neigh = KNeighborsClassifier(n_neighbors=5).fit(X_train, y_train)

  #Produce an accuracy score
  current_score = bc_neigh.score(X_test, y_test)
  print(current_score)
...
bc_neigh = KNeighborsClassifier(n_neighbors=5).fit(X, y)

#Produce an accuracy score
cv_scores = cross_val_score(bc_neigh, X, y, cv=10)
print (cv_scores)
print (cv_scores.mean())
print (cv_scores.std())
...
bc_neigh = KNeighborsClassifier(n_neighbors=5).fit(X_train, y_train)
#tree.plot_tree(bc_neigh)
results = bc_neigh.predict(X_test)
#print(y_test)
...
#
# Z = bc_neigh.predict(np.c_[xx.ravel(), yy.ravel()]) # Here we use the tree
