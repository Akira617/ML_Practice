...
class ClassifierAgent(Agent):
    ...
    def train_random_forest(self):
        ...
        # Use all attributes for each trees, but might cause overfitting
    ...
    def evaluation(self):
        ...
        for index in range(len(self.data)):
            values, freq  = self.inference_RF(self.data[index])
            best_index = np.argmax(freq)
            best_number = values[best_index]
            if self.target[index] == best_number:
                hit += 1
        length = float(len(self.data))
        print(hit)
        print(length)
        print(hit/length)
